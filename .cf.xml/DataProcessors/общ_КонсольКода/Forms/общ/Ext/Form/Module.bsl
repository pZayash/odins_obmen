...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// 20230830 Заяш
	общ_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка);
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	// 20221003 Заяш
	Если ЗначениеЗаполнено(ИмяСобытияЗавершенияРедактированияКода) Тогда
		Возврат;
	...
	Если Представление = Неопределено Тогда
		...
		Попытка
			Если Не Значение.Пустая() Тогда
				Ссылка = общ.НавигационнаяСсылка(Значение);
				Если htmlTag Тогда
			...
			КонецЕсли;
		Исключение
			Представление = общ.Ошибка_КраткоеПредставление(ИнформацияОбОшибке());
		КонецПопытки;
...
Функция ПолучитьОписаниеПеременной(ИдентификаторПеременной, ИмяПеременной, ПутьКДанным, АдресХраненияПеременных)
	...
	Исключение
		...
		общ.Ошибка_ВЖурналРегистрации("общ.КонсольКода", инфоОшибки);
	КонецПопытки
...
&НаСервере
Процедура общ_ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)

	// 20210611 Заяш 
	Если ЗначениеЗаполнено(Параметры.РедактируемыйТекст) Тогда
		РедактируемыйТекст = Параметры.РедактируемыйТекст;
		
	КонецЕсли; 
	
	// 20220612 Заяш 
	Если ЗначениеЗаполнено(Параметры.Текст1) Тогда
		Текст1 = Параметры.Текст1;
		
	КонецЕсли;
	
	// 20220612 Заяш 
	Если ЗначениеЗаполнено(Параметры.Текст2) Тогда
		Текст2 = Параметры.Текст2;
		
	КонецЕсли;
	
	// 20220612 Заяш 
	Если ЗначениеЗаполнено(Параметры.КодКонтекста) Тогда
		КодКонтекста = Параметры.КодКонтекста;
	...
	// 20210612 Заяш 
	Если 
			Параметры.Свойство("ИмяСобытияЗавершенияРедактированияКода")
		И	ЗначениеЗаполнено(Параметры.ИмяСобытияЗавершенияРедактированияКода) 
	Тогда
		ИмяСобытияЗавершенияРедактированияКода = Параметры.ИмяСобытияЗавершенияРедактированияКода;
		Элементы.ФормаЗакончитьРедактирование.Видимость = Истина;
		
	КонецЕсли;
	
КонецПроцедуры // общ_ПриСозданииНаСервере

&НаКлиенте
Процедура общ_ПриОткрытии(Отказ)

	// 20210612 Заяш 
	УстановитьРедактируемыйТекст();		

КонецПроцедуры

// 20210612 Заяш 
&НаКлиенте
Процедура УстановитьРедактируемыйТекст()
	
	Попытка	
		Если ЗначениеЗаполнено(Текст1) И ЗначениеЗаполнено(Текст2) Тогда
			СравнитьТексты(Текст1, Текст2);
			
		Иначе
			УстановитьТекст(РедактируемыйТекст, Неопределено, Ложь);
			
		КонецЕсли;
		
	Исключение
		ПодключитьОбработчикОжидания("УстановитьРедактируемыйТекст", 0.1, Истина);
...
// 20210613 Заяш 
&НаСервереБезКонтекста
Процедура СинтаксическийКонтрольНаСервере(пТекстДляКонтроля, пКодКонтекста = "")

	// Заполнить переменные контекста
	Если Не ПустаяСтрока(пКодКонтекста) Тогда 
		Попытка
			Выполнить("Если Ложь Тогда " + пКодКонтекста + " КонецЕсли;");
		Исключение 
			инфоОшибки = ИнформацияОбОшибке();
			стрОшибка = общ.Ошибка_ПодробноеПредставление(инфоОшибки);
			сообщение = Новый СообщениеПользователю;
			сообщение.Текст = "Проверка невозможна, не удалось вычислить контекст";
			сообщение.Сообщить();
			Возврат;
		КонецПопытки;
	...
	Попытка
		...
		Выполнить(
			СтрШаблон(
				"Если Ложь Тогда %1 %2
				|КонецЕсли;",
				СтрЗаменить(пКодКонтекста, Символы.ПС, " "), // %1
				пТекстДляКонтроля // %2
				)
			);
		сообщение = Новый СообщениеПользователю;
		сообщение.Текст = "Синтаксических ошибок не обнаружено!";
		сообщение.Сообщить();

	Исключение
		иО = ИнформацияОбОшибке();
		сообщение = Новый СообщениеПользователю;
		сообщение.Текст = общ.Ошибка_КраткоеПредставление(иО);
		сообщение.Сообщить();
...
Процедура СинтаксическийКонтроль(Команда)
	...
	СинтаксическийКонтрольНаСервере(ПолучитьТекст(), КодКонтекста);
	
КонецПроцедуры

// 20220929 Заяш
&НаКлиенте
Процедура СравнитьТексты(пТекст1, пТекст2, пПодсветкаСинтаксиса = Ложь) 

	View().setText(пТекст1); 

	//https://github.com/salexdv/bsl_console/blob/develop/docs/compare.md
	...
	//markLines - boolean, включена/выключена индикация (пиктограмма) для измененных строк. 
	//	Необязательный параметр. Значение по умолчанию: true	
	//	
	View().compare(
		пТекст2, //text - string, текст для сравнения с текущим текстом редактора
		Истина, //sideBySide - boolean, двусторонний/построчный режим сравнения
		пПодсветкаСинтаксиса, //highlight - boolean, включена/выключена подсветка синтаксиса
		Истина //markLines - boolean, включена/выключена индикация (пиктограмма) для измененных строк. 
		); 
...
Процедура ЗакончитьРедактирование(Команда)
	...
	Если ЗначениеЗаполнено(ИмяСобытияЗавершенияРедактированияКода) Тогда
		Оповестить(ИмяСобытияЗавершенияРедактированияКода, ПолучитьТекст());
