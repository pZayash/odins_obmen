...
Функция ИсходящееСообщение_Отправить(пИсходящееСсылка, пУзелПолучатель) Экспорт
	...
	Если ПустаяСтрока(пУзелПолучатель) Тогда
	...
	обм.УзелПривести(пУзелПолучатель);
	РеализацияИнтерфейса.ИсходящееСообщение_Отправить(пИсходящееСсылка, пУзелПолучатель);
