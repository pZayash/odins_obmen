...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПустаяСтрока(Параметры.Пользователь) Тогда
		...
		Если ПустаяСтрока(ИмяПользователя) Тогда
			ПоПользователю.Представление = общ.ПолноеИмяНеУказанногоПользователя();
		Иначе
	...
	Если Параметры.Свойство("ОтбиратьОшибки") Тогда
		ОтборЖурналаРегистрации.Вставить("Уровень", "Ошибка");
		
	КонецЕсли;

	Если Параметры.Свойство("Данные") И ЗначениеЗаполнено(Параметры.Данные) Тогда
...
Процедура ПросмотрТекущегоСобытияВОтдельномОкне()
	...
	ОткрытьФорму("Обработка.общ_ЖурналРегистрации.Форма.ФормаСобытия", ПараметрыФормы);
...
Процедура УстановитьОтбор()
	...
	ФормаОтбора = ОткрытьФорму("Обработка.общ_ЖурналРегистрации.Форма.ОтборЖурналаРегистрации",
...
Процедура ПрочитатьЖурнал(Знач ОтборЖурналаНаКлиенте)
	...
	Для Каждого ЭлементОтбора Из ОтборЖурналаНаКлиенте Цикл
		Если ЭлементОтбора.Ключ = "Уровень" Тогда
			...
			Если ЭлементОтбора.Значение = "Ошибка" Тогда
				СписокУровней.Добавить("Ошибка", "Ошибка", Истина);
	...
	Для Каждого СобытиеЖурнала Из СобытияЖурнала Цикл
		...
		ИначеЕсли СобытиеЖурнала.ИмяПользователя = "" Тогда
			СобытиеЖурнала.ИмяПользователя = общ.ПолноеИмяНеУказанногоПользователя();
...
Процедура ПреобразованиеЭлементаОтбора(Отбор, ЭлементОтбора)
	...
	Для Каждого ЗначениеИзСписка Из ЭлементОтбора.Значение Цикл
		Если ВРег(ЭлементОтбора.Ключ) = ВРег("Уровень") Тогда
			...
			НовоеЗначение.Добавить(Обработки.общ_ЖурналРегистрации.УровеньЖурналаРегистрацииЗначениеПоИмени(ЗначениеИзСписка.Значение));
		ИначеЕсли ВРег(ЭлементОтбора.Ключ) = ВРег("СтатусТранзакции") Тогда
			...
			НовоеЗначение.Добавить(Обработки.общ_ЖурналРегистрации.СтатусТранзакцииЗаписиЖурналаРегистрацииЗначениеПоИмени(ЗначениеИзСписка.Значение));
		Иначе
