...
Функция ИдентификаторИнформационнойБазы() Экспорт
	...
	текущаяСтрокаСоединения = НРег(общ.Файл_ПодготовитьИмя(СтрокаСоединенияИнформационнойБазы()));
	идГодный = Не параметрыИд.Отказ И параметрыИд.Имя = текущаяСтрокаСоединения;
	...
	новыйИд = общ.Файл_ИмяДополненноеИдентификатором(
						текущаяСтрокаСоединения, Строка(Новый УникальныйИдентификатор()), "dt");
