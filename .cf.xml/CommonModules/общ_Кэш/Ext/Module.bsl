...
Функция ПодсистемаСуществует(пПолноеИмяПодсистемы) Экспорт 
	
	Если ПустаяСтрока(пПолноеИмяПодсистемы) Тогда
		Возврат Ложь;
		
	КонецЕсли;
	
	// Если есть БСП, вызвать ее метод
	базоваяПодсистемаБСП = "СтандартныеПодсистемы.БазоваяФункциональность";
	Если пПолноеИмяПодсистемы <> базоваяПодсистемаБСП И ПодсистемаСуществует(базоваяПодсистемаБСП) Тогда
		// Обманка для контекстной подсказки и проверки модуля
		Если Ложь Тогда
			ОбщегоНазначения = Неопределено; 
		КонецЕсли;
		
		Возврат ОбщегоНазначения.ПодсистемаСуществует(пПолноеИмяПодсистемы);

	КонецЕсли;
	
	частиИмени = общ.Строка_РазделитьСокрЛП(пПолноеИмяПодсистемы);
	...
	Для Каждого имяПодсистемы Из частиИмени Цикл
		Если местоПоиска = Неопределено Тогда
			местоПоиска = Метаданные.Подсистемы.Найти(имяПодсистемы);
			
		Иначе
			местоПоиска = местоПоиска.Найти(имяПодсистемы);
	...
	подсистемаНайдена = местоПоиска <> Неопределено;
	
	Возврат подсистемаНайдена;
