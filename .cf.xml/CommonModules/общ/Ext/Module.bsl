Процедура ВЖурнал(пИмяСобытия, пКомментарий, Знач пУровеньЖурнала = Неопределено, Знач пСсылкиДанных = Неопределено) Экспорт
	...
	Для Каждого данные Из ссылкиДанных Цикл
		данныеСсылка = общ.Ссылка_ПоОбъекту(данные);
...
Функция Объект_ПоТипу(пТипСсылкиОбъекта, пСсылка = Неопределено, Знач пУИДКандидат = Неопределено) Экспорт
...
Функция Коллекция_ПоСсылке(пСсылкаИлиОбъект, Знач пИсключаяСвойства = Неопределено) Экспорт
	...
	Иначе // Не перечисление
		образ.Вставить("_ИмяТаблицы", Объект_ИмяТаблицы(ссылкаОбъекта));
...
// @TODO 250728 Заяш: Формат Альфа, перенесена в модуль обм, здесь оставлена для совместимости. Найти и удалить
...
Функция Файл_ПараметрыИмени(Знач пИмяФайла) Экспорт
	...
	ЭтоДанныеXML = Файл_Расширение = ".xml.txt"; // ИмяФайла.guid.xml.txt
...
Функция JSON(пЗначение, пЭкранироватьСимволы = Истина, Знач пОтступ = "", пКэш = Неопределено) Экспорт
	...
	ИначеЕсли общ.ЭтоСсылка(пЗначение) Тогда
		...
		общ.Коллекция_РазвернутьСсылки(образСсылки, 1, "__Реквизиты__"); 
