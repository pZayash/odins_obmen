...
Функция Ошибка_КраткоеПредставление(пИнфоОшибки) Экспорт 
	...
	Если ВерсияПлатформыНеНижеУказанной("8.3.17") Тогда
...
Функция Проверка_Тип(пЗначение, Знач пПроверяемыйТип, пВызыватьИсключение = Истина) Экспорт
	...
	Если Не ожидаемыйТипПараметра2.СодержитТип(ТипЗнч(пПроверяемыйТип)) Или пПроверяемыйТип = Неопределено Тогда
		ВызватьИсключение 
			СтрШаблон(
