...
// Если передан массив, вернет его
//
// Параметры:
//  пЗначение	 - произвольный
// 
// Возвращаемое значение:
//  массив - значение, помещенное в массив
//
Функция Массив_ПоЗначению(пЗначение, пТипыДляОтбораЭлементов = Неопределено, пРазделительЗначений = Неопределено) Экспорт
...
Функция Объект_ПоТипу(пТипСсылкиОбъекта, пСсылка = Неопределено, Знач пУИДКандидат = Неопределено, пЭтоГруппа = Неопределено) Экспорт
...
Функция Ссылка_ПоКоллекции(пКоллекция) Экспорт 
	...
	Если общ.Коллекция_Свойство(пКоллекция, "_Тип", стрТипСсылки) Тогда
		типСсылки = Тип_ПоСтроке(стрТипСсылки);		
	КонецЕсли;
...
КонецФункции 
	...
	// Вспомогательная функция для метода ТабличныйДокумент_ИменаПараметров
