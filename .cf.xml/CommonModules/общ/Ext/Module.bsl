Функция Base64_ВСтроку(пСтрокаBase64) Экспорт
...
Функция Группа_Получить(пИмяГруппы, пТип, пВладелец = Неопределено) Экспорт
	...
	имяГруппы = ИнформационнаяБазаИмя(пИмяГруппы);
...
Функция ЗначениеВКоллекцию(пЗначение) Экспорт 
	...
	ИначеЕсли Не ПустаяСтрока(пЗначение) Тогда
		массивИсключаемыхСвойств = СтрРазделить(пЗначение, ",", Ложь);
		Для Каждого имяСвойства Из массивИсключаемыхСвойств Цикл
...
// 20270814
Функция ИмяТаблицыВТипСсылки(пИмяТаблицы) Экспорт
...
// 20230713
Функция ИнформационнаяБазаИмя(Знач пСтрокаСоединенияИнформационнойБазы = Неопределено) Экспорт 
...
//  Булево.
//
Функция ИнформационнаяБазаФайловая(Знач пСтрокаСоединенияИнформационнойБазы = "") Экспорт
...
Функция Коллекция_ЗаписатьJSONсСортировкой(пКоллекция, пКэш = Неопределено) Экспорт
	...
	Если этоСписокЗначений Тогда
		...
		стрJSON = ЗначениеВJSON(пКоллекция, Ложь, , пКэш);
	...
	Для Каждого элемент Из пКоллекция Цикл
		...
		Иначе
			элемент_JSON = ЗначениеВJSON(элемент_значение, , , пКэш);
	...
	стрJSON = ЗначениеВJSON(упорядоченнаяКоллекция, Ложь, , пКэш);
...
Функция КоллекцияВСсылку(пКоллекция) Экспорт 
	...
	Если общ.Коллекция_Свойство(пКоллекция, "_Тип", стрТипСсылки) Тогда
		типСсылки = СтрокаВТип(стрТипСсылки);		
	КонецЕсли;
	...
	// 0. Значение перечисления
		...
		Если этоПеречисление Тогда
			перечисление_менеджер = общ.Менеджер_Получить(типСсылки);
	...
	Если естьНавигационнаяСсылка Тогда	
		ссылкаРезультат = общ.НавигационнаяСсылкаВСсылку(навигационнаяСсылка); 
	...
	менеджер = общ.Менеджер_Получить(типСсылки); 
...
// Функция для получения значения из коллекции
// Сокращенная обертка над Коллекция_ПолучитьЗначениеПоПути 
Функция Коллекция_Значение(пКоллекция, пПуть) Экспорт
	
	результат = Коллекция_ПолучитьЗначениеПоПути(пКоллекция, пПуть);
...
КонецФункции // Коллекция_Значение

Функция Коллекция_ПолучитьЗначениеПоПути(пКоллекция, Путь, Разделитель = ".") Экспорт 
...
Функция Коллекция_РазвернутьСсылки(пКоллекция, Знач пУровень, Знач пИсключаяСвойства = Неопределено) Экспорт
	...
	коллекцияИсключаемых = ЗначениеВКоллекцию(пИсключаяСвойства);
	...
	ссылкаИзКоллекции = общ.КоллекцияВСсылку(пКоллекция);
	...
	Для Каждого пара Из пКоллекция Цикл 
		...
		Если общ.ЭтоСсылка(пара.Значение) Тогда
			...
			образСсылки = общ.СсылкаВКоллекцию(разворачиваемяСсылка, стрИсключитьСвойства);
...
Функция Коллекция_Свойство(пКоллекция, пКлюч, пНайденноеЗначение = Неопределено) Экспорт
	...
	ЭтоКоллекция = Проверка_Тип(пКоллекция, "Структура, Соответствие, Массив, ФиксированнаяСтруктура, ФиксированноеСоответствие", Ложь) = ""; 
	...
	Если ТипЗнч(пКоллекция) = Тип("Массив") Тогда 
		...
		ключИндекс = Неопределено; 
		
		Если ТипЗнч(пКлюч) = Тип("Число") Тогда
			ключИндекс = пКлюч;

		ИначеЕсли ТипЗнч(пКлюч) = Тип("Строка") Тогда
			Попытка
				ключИндекс = Число(пКлюч);
			Исключение
				Возврат Ложь;
			КонецПопытки;
		...
		Если ТипЗнч(ключИндекс) <> Тип("Число") Тогда
			Возврат Ложь;  
			
		КонецЕсли;

		ключИндекс =  Цел(ключИндекс);
	
		естьЭлементМассива = пКоллекция.Количество() > ключИндекс;  
		
		Если Не естьЭлементМассива Тогда
			Возврат Ложь;
			
		КонецЕсли;
		
		пНайденноеЗначение = пКоллекция[ключИндекс];
	...
	ИначеЕсли ТипЗнч(пКоллекция) = Тип("Соответствие") Или ТипЗнч(пКоллекция) = Тип("ФиксированноеСоответствие") Тогда
	    ...
	    КоличествоЭлементов = пКоллекция.Количество();
		
		пКоллекция.Удалить(пКлюч);
	    
	    Если КоличествоЭлементов = пКоллекция.Количество() Тогда
	        Возврат Ложь;
...
// 
Функция Менеджер_ПолучитьБитуюСсылку(пМенеджер, пКандидат = Неопределено) Экспорт
	
	этоПеречисление = общ.КорневойТипКонфигурации_Получить(пМенеджер) = "Перечисление";
	Если этоПеречисление Тогда
		Возврат Неопределено;
	...
	Пока Истина Цикл
		...
		Если СсылкаСуществует(ссылкаРезультат) Тогда
			Продолжить;
...
Функция Менеджер_СоздатьОбъект(пМенеджер, Знач пСоздатьГруппу = Ложь) Экспорт 
	...
	корневойТип = КорневойТипКонфигурации_Получить(пМенеджер.ПустаяСсылка());
...
//
Функция НавигационнаяСсылкаВСсылку(пНавигационнаяСсылка) Экспорт
...
Функция Объект_ИмяТаблицы(пОбъект) Экспорт
	...
	Попытка 
		...
		ИначеЕсли ТипЗнч(пОбъект) = Тип("Тип") Тогда
			результат = Общ.Менеджер_Получить(пОбъект).ПустаяСсылка().Метаданные().ПолноеИмя();
	...
	Исключение
		...
		обОшибке = ОбработкаОшибок.ПодробноеПредставлениеОшибки(инфоОшибки);
		результат = Неопределено;
...
//
Функция Объект_ПолучитьКоллекциюСсылокВОбъекте(пОбъектСсылочный, пОписаниеТиповДляРекурсивнойОбработки = Неопределено, пКоллекцияОбработанных = Неопределено, КоллекцияСсылокВОбъекте = Неопределено) Экспорт 
	...
	Для Каждого Реквизит Из МетаданныеОбъекта.Реквизиты Цикл
		...
		Если общ.ОбъектСсылкаДругое(ЗначениеРеквизита) <> "Другое" И ЗначениеЗаполнено(ЗначениеРеквизита.Ссылка) Тогда
			...
			Если ВыгружатьПоСсылке Тогда
				Объект_ПолучитьКоллекциюСсылокВОбъекте(
					ЗначениеРеквизита,
...
Функция Объект_ПолучитьПоТипу(пТипСсылкиОбъекта, пСсылка = Неопределено, Знач пУИД = Неопределено) Экспорт
	
	Проверка_Тип(пТипСсылкиОбъекта, Тип("Тип"));

	результатОбъект = Неопределено;
	менеджер = Менеджер_Получить(пТипСсылкиОбъекта);
	
	Если общ.ЭтоЗаполненнаяСсылка(пСсылка, пТипСсылкиОбъекта)  Тогда
		Если общ.СсылкаСуществует(пСсылка) Тогда
...
Функция ОбъектСсылкаДругое(пОбъект) Экспорт
	...
	Если КорневойТипКонфигурации_Получить(ОбъектМетаданных) = Неопределено Тогда
		Возврат ВариантыРезультата.Другое;
...
Функция ОписаниеТиповСтроки_Получить(ДлинаСтроки = 1000) Экспорт
...
Функция НавигационнаяСсылка(пОбъект) Экспорт
	...
	ссылкаНаОбъект = Ссылка_Получить(пОбъект);
...
Функция СоответсвиеВСтруктуру(пСоответсвие) Экспорт 
	
	общ.Проверка_Тип(пСоответсвие, "Соответствие");

	структураРезультат = Новый Структура; 
	
	текстJSON = общ.ЗначениеВJSON(пСоответсвие);
...
Функция СсылкаВКоллекцию(пСсылкаИлиОбъект, Знач пИсключаяСвойства = Неопределено) Экспорт
	...
	ИначеЕсли объектСсылкаДругое = "Ссылка" Тогда
		ссылкаОбъекта = пСсылкаИлиОбъект;
		этотОбъектУдален = общ.ЭтоЗаполненнаяСсылка(ссылкаОбъекта) И Не СсылкаСуществует(ссылкаОбъекта);

	ИначеЕсли объектСсылкаДругое = "Объект" Тогда
		ссылкаОбъекта = пСсылкаИлиОбъект.Ссылка;
		этоНовый = пСсылкаИлиОбъект.ЭтоНовый();
		этотОбъектУдален = Не этоНовый И общ.ЭтоЗаполненнаяСсылка(ссылкаОбъекта) И Не СсылкаСуществует(ссылкаОбъекта);
	...
	образ.Вставить("_Тип", Тип_ПолучитьСтроковоеПредставление(типОбъекта)); 
	Если Не Тип_ЭтоСсылка(типОбъекта) Тогда
		Возврат образ;
	...
	Если этоСпр Тогда
		...
		Если исключитьВложенныеСсылки Или образ["Владелец"] = ссылкаОбъекта Тогда
			образ.Вставить("Владелец", общ.СсылкаВКоллекцию(образ["Владелец"], "__Все__"));
	...
	Если естьИерархия Тогда
		...
		Если исключитьВложенныеСсылки Или образ["Родитель"] = ссылкаОбъекта Тогда
			образ.Вставить("Родитель", общ.СсылкаВКоллекцию(образ["Родитель"], "__Все__"));
	...
	Для Каждого реквизитМетаданных Из мета.Реквизиты Цикл
		...
		Если выгрузкаРазрешена Тогда
			...
			Если значениеРеквизита = ссылкаОбъекта Тогда
				образ.Вставить(реквизитМетаданных.Имя, общ.СсылкаВКоллекцию(значениеРеквизита, "__Все__")); 
			...
			Для Каждого рядТЧ Из ТЧ Цикл
				...
				Для Каждого реквизитТчМетаданных Из тчМетаданных.Реквизиты Цикл
					...
					Если общ.ЭтоСсылка(типРеквизитаТЧ) И (исключитьВложенныеСсылки Или значениеРеквизита = ссылкаОбъекта) Тогда
						образСтрокиТЧ.Вставить(реквизитТчМетаданных.Имя, общ.СсылкаВКоллекцию(значениеРеквизитаТЧ, "__Все__"));
	...
	Если этоПланСчетов Тогда
		...
		Если Не общ.Коллекция_Свойство(коллекцияИсключаемых, НРег("ВидыСубконто")) Тогда
			...
			Для индексСубконто = 0 По ссылкаОбъекта.ВидыСубконто.Количество() - 1 Цикл
				...
				образВидаСубконто = СсылкаВКоллекцию(строкаВидовСубконто.ВидСубконто);
...
Функция Ссылка_ИмяТаблицы(пСсылка) Экспорт
	...
	// Старый вариант
	Возврат СтрЗаменить(Тип_ПолучитьСтроковоеПредставление(пСсылка),"Ссылка.",".");
	
КонецФункции

Функция Ссылка_Получить(пОбъект) Экспорт
...
Функция СсылкаСуществует(пСсылка) Экспорт 
	...
	корневойТип = Общ.КорневойТипКонфигурации_Получить(пСсылка);
    ...
    Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("СсылкаНаОбъект", пСсылка);
	Запрос.Текст = 
		СтрШаблон(
	...
	// При активной транзакции выполнять через попытку
	// При работе фоновых заданий, в транзакии случаются ошибки, тогда данный запрос упадет с ошибкой
	//  
...
Функция СтрокаВТип(пИмяТипа) Экспорт
...
Функция ТабличныйДокумент_ПолучитьМассивИменПараметров(пТабДок, пЗаполнениеТекстКакШаблон = Ложь, пТегСлева = "[", пТегСправа = "]") Экспорт
	...
	Если ТипЗнч(пТабДок) = Тип("ОбластьЯчеекТабличногоДокумента") Тогда
		Возврат 
			ОбластьТабличногоДокумента_ПолучитьМассивИменПараметров(
...
Функция ОбластьТабличногоДокумента_ПолучитьМассивИменПараметров(пЯчейка, пЗаполнениеТекстКакШаблон = Ложь, пТегСлева = "[", пТегСправа = "]") 
...
Функция ТекущаяДатаНаСервере()
	...
	Возврат общ_Сервер.ТекущаяДатаНаСервере();
...
Функция Тип_ПолучитьПустоеЗначение(ЗаданныйТип) Экспорт
	...
	ИначеЕсли ТипЗнч(ЗаданныйТип) = Тип("ОписаниеТипов") Тогда
		Если ЗаданныйТип.Типы().Количество() = 0 Тогда
			Возврат Неопределено;
		Иначе
			Возврат Тип_ПолучитьПустоеЗначение(ЗаданныйТип.Типы()[0]);
		КонецЕсли;
...
Функция UnJSON(СтрJSON, ПрочитатьВСоответствие = Истина) Экспорт
	...
	ПреобразованиеВыполнено = Истина;
...
Функция ЗначениеВJSON(пЗначение, пЭкранироватьСимволы = Истина, Знач пОтступ = "", пКэш = Неопределено) Экспорт
	...
	Если типЗначения = Тип("Строка") Тогда
		Если пЭкранироватьСимволы Тогда
			стр = """" + ЭкранироватьНедопустимыеСимволыJSON(пЗначение) + """";
		...
		Для Каждого ЭлементМассива Из пЗначение Цикл
			стр = 
				СтрШаблон(
					...
					пОтступ, // %2
					ЗначениеВJSON(ЭлементМассива, пЭкранироватьСимволы, отступСледующего, пКэш) // %3
					); 
	...
	ИначеЕсли типЗначения = Тип("СписокЗначений") Тогда
		...
		Для Каждого элемент Из пЗначение Цикл 
			...
			значение = ЗначениеВJSON(элемент.Значение, пЭкранироватьСимволы, отступСледующего, пКэш);
	...
	ИначеЕсли типЗначения = Тип("ТаблицаЗначений") Тогда
		...
		стр = ЗначениеВJSON(МассивОбразовСтрокТЗ, пЭкранироватьСимволы, отступСледующего, пКэш);
	...
	ИначеЕсли общ.ЭтоСсылка(пЗначение) Тогда
		// Создать краткий образ ссылки без реквизитов, только необходимые для идетнификации ссылки поля
 		образСсылки = общ.СсылкаВКоллекцию(пЗначение, "__Реквизиты__");
...
Функция ЭкранироватьНедопустимыеСимволыJSON(пСтр)
...
Функция ПодписьSHA256RSA_Получить(пДляПодписи, пЗакрытыйКлючPEM) Экспорт
...
// Из модуля ВариантыОтчетов
#Область ВариантыОтчетов

// Запись в журнал регистрации.
Процедура ВариантыОтчетов_ЗаписатьВЖурнал(Уровень, Сообщение, ВариантОтчета = Неопределено) Экспорт
	...
	КонецЕсли;
	ЗаписьЖурналаРегистрации(ВариантыОтчетов_НаименованиеПодсистемы(),
		Уровень, ОбъектМетаданных, Данные, Сообщение);
КонецПроцедуры

Функция ВариантыОтчетов_НаименованиеПодсистемы()
...
Процедура ЗаполнитьОписаниеСтрокиВарианта(ОписаниеВарианта, ОписаниеОтчета)
	...
	ОписаниеВарианта.Размещение = общ.СкопироватьРекурсивно(ОписаниеОтчета.Размещение);
	ОписаниеВарианта.ФункциональныеОпции = общ.СкопироватьРекурсивно(ОписаниеОтчета.ФункциональныеОпции);
	ОписаниеВарианта.НастройкиДляПоиска = общ.СкопироватьРекурсивно(ОписаниеОтчета.НастройкиДляПоиска);
...
Функция КоллекцияПредопределенныхВариантовОтчетов()
	...
	Результат.Колонки.Добавить("Наименование", общ.ОписаниеТиповСтроки_Получить());
	Результат.Колонки.Добавить("Описание", общ.ОписаниеТиповСтроки_Получить());
	...
	Результат.Колонки.Добавить("Тип", общ.ОписаниеТиповСтроки_Получить());
	...
	Результат.Колонки.Добавить("КлючЗамеров", общ.ОписаниеТиповСтроки_Получить());
...
Функция ОписаниеОтчета(Настройки, Отчет) Экспорт
	...
	Если Результат.Количество() <> 1 Тогда
		ВызватьИсключение СтрШаблон(
			...
			"Отчет", "ВариантыОтчетов.ОписаниеОтчета", Строка(Отчет), ВариантыОтчетов_НаименованиеПодсистемы());
	КонецЕсли;
...
Функция ТипСсылкиДополнительногоОтчета()
	Если Метаданные.Справочники.Найти("ДополнительныеОтчетыИОбработки") Тогда
...
Процедура УстановитьРежимВыводаОтчетаВПанеляхОтчетов(Настройки, Отчет, ГруппироватьПоОтчетам)
	...
	Иначе
		...
		Если ОписаниеОтчета.Количество() <> 1 Тогда
			ВариантыОтчетов_ЗаписатьВЖурнал(
				УровеньЖурналаРегистрации.Предупреждение, 
				СтрШаблон(НСтр("ru = 'Отчет ""%1"" не подключен к подсистеме.'"), Отчет.Имя)
				);
			Возврат;
		КонецЕсли;
...
Функция СоответсвиеИсключаемыхСвойств_УбратьУровеньДляРекурсии(пСоотвИсключаемыхСвойств, пПрефиксСвойстваРодителя)
	
	соотвИсключаемыхСвойствРекурсия = Новый Соответствие;
	
	Если ПустаяСтрока(пПрефиксСвойстваРодителя) Тогда
		Возврат соотвИсключаемыхСвойствРекурсия;
		
	КонецЕсли;
	
	Для Каждого пара Из пСоотвИсключаемыхСвойств Цикл
		стрПрефикс = НРЕГ(пПрефиксСвойстваРодителя + ".");
		Если СтрНайти(пара.Ключ, стрПрефикс) = 1 Тогда
			имяСвойстваРекурсия = СтрЗаменить(пара.Ключ, стрПрефикс, ""); 
			соотвИсключаемыхСвойствРекурсия.Вставить(имяСвойстваРекурсия, имяСвойстваРекурсия);
