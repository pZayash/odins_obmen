...
Процедура ВЖурнал(пИмяСобытия, пКомментарий, Знач пУровеньЖурнала = Неопределено, 
	Знач пДанные = Неопределено) Экспорт
	...
	Если ТипЗнч(пДанные) = Тип("Массив") Тогда
		данныеМассив = пДанные;
		
	ИначеЕсли пДанные = Неопределено Тогда
		// Ничего не делать
		
	ИначеЕсли Не ЗначениеЗаполнено(пДанные) Тогда
		// Ничего не делать
		
	Иначе
		данныеМассив.Добавить(пДанные);
		
	КонецЕсли;
	
	Если ТранзакцияАктивна() Или данныеМассив.Количество() = 0 Тогда
	...
	Для Каждого данныеЗначение Из данныеМассив Цикл
		уникальныеДанные.Вставить(данныеЗначение, данныеЗначение);
		
	КонецЦикла;
	данныеЗначение = Неопределено;
	
	// Доп записи для связанных с ошибкой данных
	Для Каждого пара Из уникальныеДанные Цикл
		данныеЗначение = пара.Значение;
		мета = Неопределено;
		Если Не ТранзакцияАктивна() Тогда
			...
			данныеСсылка = общ.Ссылка_ПоОбъекту(данныеЗначение);
...
// см. общ.Коллекция_ЗначениеПоПути
// Это альтернативный синтаксис
Функция Коллекция_Значение(пКоллекция, пПуть, пРазделитель = ".") Экспорт
	
	Возврат Коллекция_ЗначениеПоПути(пКоллекция, пПуть, пРазделитель);
...
Функция Коллекция_Свойство(пКоллекция, пКлюч, пНайденноеЗначение = Неопределено) Экспорт
	...
	ИначеЕсли 	ТипЗнч(пКоллекция) = Тип("Структура") 
		Или 	ТипЗнч(пКоллекция) = Тип("ФиксированнаяСтруктура")
		Или 	ТипЗнч(пКоллекция) = Тип("ДанныеФормыСтруктура")
		Тогда
...
//
Функция КорневойТип_ЭтоСсылочный(пКорневойТип) Экспорт
...
Функция Менеджер_ПоОбъекту(Знач пОбъект) Экспорт
	...
	метаОбъекта = Неопределено;
	Если ТипЗнч(пОбъект) = Тип("Тип") Тогда
		метаОбъекта = Метаданные.НайтиПоТипу(пОбъект);
		
	ИначеЕсли ТипЗнч(пОбъект) = Тип("ОбъектМетаданных") Тогда
		метаОбъекта = пОбъект;
		
	Иначе
		метаОбъекта = Метаданные.НайтиПоТипу(ТипЗнч(пОбъект));
		
	КонецЕсли;
	
	Если ТипЗнч(метаОбъекта) <> Тип("ОбъектМетаданных") Тогда
		Возврат Неопределено;
		
	КонецЕсли;
	
	корневойТип = общ.КорневойТипКонфигурации(метаОбъекта);
	...
	ИначеЕсли корневойТип = "Обработка" Тогда
		Возврат Обработки[имяОбъекта];
		
	ИначеЕсли корневойТип = "Перечисление" Тогда
		Возврат Перечисления[имяОбъекта];
		
	ИначеЕсли корневойТип = "ПланВидовРасчетов" Тогда
...
Процедура Ошибка_ВЖурналРегистрации(пИмяСобытия, пКомментарийОшибки, Знач пДанные = Неопределено) Экспорт
	...
	ВЖурнал(пИмяСобытия, пКомментарийОшибки, УровеньЖурналаРегистрации.Ошибка, пДанные);
...
// Возвращает табличный документ, сформированный по таблице значений.
// Первая строка - имена колонок, далее - значения строк.
// Использует ПостроительОтчета без шапки и подвала (совместимость с ПостроительЗапроса при чтении XLSX).
//
// См. ТабличныйДокумент_ВТаблицуЗначений - обратное преобразование (чтение из области ячеек).
//
// Параметры:
//   пТЗ - ТаблицаЗначений - исходная таблица.
//
// Возвращаемое значение:
//   ТабличныйДокумент - документ в режиме только просмотра.
//
Функция ТаблицаЗначений_ВТабличныйДокумент(пТЗ) Экспорт

	ТабДок = Новый ТабличныйДокумент;

	Построитель = Новый ПостроительОтчета;
	Построитель.ИсточникДанных = Новый ОписаниеИсточникаДанных(пТЗ);
...
Функция Тип_ПоМетаданным(пМетаданные) Экспорт
	...
	Если КорневойТип_ЭтоСсылочный(КорневойТип) Тогда 
...
Функция ЭтоКоллекцияКлючЗначение(пЗначение) Экспорт 
	...
	Возврат
		...
		Или типЗначения = Тип("ДанныеФормыСтруктура")
		;
...
// Устарела, не использовать, см. КорневойТип_ЭтоСсылочный
//
Функция КорневойТипКонфигурации_Проверить(пКорневойТип) Экспорт

	Возврат КорневойТип_ЭтоСсылочный(пКорневойТип);
