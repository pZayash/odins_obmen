...
Функция Ссылка_ПоОбразуВФорматеОбмАльфа(пОбразАльфа) Экспорт
	...
	Если этоПеречисление Тогда
		...
		перечисление_существует = перечисление_метаданные.ЗначенияПеречисления.Найти(пОбразАльфа["_Значение"]) <> Неопределено;
		Если перечисление_существует Тогда
			Возврат перечисление_менеджер[пОбразАльфа["_Значение"]];
	...
	уид = Неопределено;
	Если общ.Коллекция_Свойство(пОбразАльфа, "_УникальныйИдентификатор", уид) И общ.Строка_ЭтоУникальныйИдентификатор(уид) Тогда
...
Функция Отправить_HTTP_POST(пСообщение, пПараметры, пРезультат = Неопределено)
	...
	Попытка
		...
		пРезультат = ответ.ПолучитьТелоКакСтроку(кодировка);
...
// @TODO 250724 Заяш: Параметр пУзелОтправитель используется только в методе ВосстановитьРеквизитОбъекта
...
// @TODO 250724 Заяш: Параметр узел тут не используется. Убрать его, проверить остальные параметры
...
Процедура ВосстановитьРеквизитОбъекта(пУзел, пОбъект, Знач пКлюч, пОбразЗначения, пИмяФункцииВосстановления = Неопределено, пМодульФункцииВосстановления = Неопределено)
	...
	Если общ.Коллекция_ЭтоОбразСсылки(пОбразЗначения) Тогда
		пОбъект[пКлюч] = общ.Ссылка_ПоКоллекции(пОбразЗначения);
		Возврат;
...
Процедура Отправка_1_ИсходящиеСообщения_Создать() Экспорт
	...
	РегистрыСведений.обм_Измененные.ОбработатьтОчередьИзмененных();
	
КонецПроцедуры

Функция Отправка_2_ИсходящиеСообщения_Отправить() Экспорт
...
Процедура ПриЗаписи(пИсточник, пОтказ = Ложь) Экспорт
	...
	интерфейс = Обработки.обм_ИнтерфейсОбработчиков_РегистрацияИзмененных.Создать();
	...
	Для Каждого обработчик Из реализацииИнтерфейса Цикл
		...
		необходимаРегистрация =
			(этоОбмен И интерфейс.ОбъектИБ_РегистрироватьПриПоступленииПоОбмену(источникСсылка, узелОтправитель))
			ИЛИ (НЕ этоОбмен И интерфейс.ОбъектИБ_РегистрироватьПриЗаписи(источникСсылка))
			;
	...
	Если необходимаРегистрация Тогда
		...
		пИсточник.ДополнительныеСвойства.Свойство("КатегорияОтправителя", категорияОтправителя);
		идентификаторОтправителя = Неопределено;
		пИсточник.ДополнительныеСвойства.Свойство("ИдентификаторОтправителя", идентификаторОтправителя);
		
		РегистрыСведений.обм_Измененные.Зарегистрировать(источникСсылка, узелОтправитель, категорияОтправителя, идентификаторОтправителя);
