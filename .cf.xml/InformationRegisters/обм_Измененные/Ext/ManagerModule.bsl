...
Процедура КОтправке(пИсточникСсылка, пВерсияДанных, пУзелОтправительИсточника = Неопределено, пУдалитьРегистрацию = Истина) Экспорт
	...
	Попытка
		исходящиеСообщения = Справочники.обм_ИсходящиеСообщения.СоздатьПоСсылкеИсточника(
			пИсточникСсылка, пУзелОтправительИсточника);
...
Процедура ОбработатьтОчередьИзмененных() Экспорт
