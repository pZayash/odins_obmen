...
Процедура Записать(пУзел, пКатегория, пИдентификатор, пПриемник, пКомментарий = Неопределено, пПакетДанных = Неопределено) Экспорт
	...
	// Запись в регистр сведений обм_Приемники должна быть только в этом месте, больше нигде
	набор = РегистрыСведений.обм_Приемники.СоздатьНаборЗаписей();
	набор.Отбор.Узел.Установить(пУзел);
	набор.Отбор.Категория.Установить(пКатегория);
	набор.Отбор.Идентификатор.Установить(пИдентификатор);
	
	// В наборе должна быть только одна запись, чтобы избежать
	запись = набор.Добавить();
	запись.Узел = пУзел;
	запись.Категория = пКатегория;
	запись.Идентификатор = пИдентификатор;
	запись.Приемник = приемникСсылка;
...
Функция Приемник_Найти(Знач пУзелИлиПакетДанных, Знач пКатегория = Неопределено, Знач пИдентификатор = Неопределено) Экспорт
	...
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ уки КАК уки
	|		ПО П.Узел = уки.Узел
	|			И П.Категория = уки.Категория
	|			И П.Идентификатор = уки.Идентификатор";
...
Функция Приемник_Ссылка(пУзел, пКатегория, пИдентификатор, пТипСсылки, пСсылкаКандидат = Неопределено) Экспорт
	...
	РегистрыСведений.обм_Приемники.Записать(пУзел, пКатегория, пИдентификатор, ссылкаПриемника, "Резерв ссылки");
