...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	общ.УстановитьРежимВыводаВПанеляхОтчетов(Настройки, НастройкиОтчета, Ложь);
	
	НастройкиВарианта = общ.ОписаниеВарианта(Настройки, НастройкиОтчета, "АнализАктивностиПользователей");
	...
	НастройкиВарианта = общ.ОписаниеВарианта(Настройки, НастройкиОтчета, "АктивностьПользователя");
	НастройкиВарианта.Описание = 
		НСтр("ru = 'Подробная информация о том,
		|с какими объектами работал пользователь в программе.'");
	
	НастройкиВарианта = общ.ОписаниеВарианта(Настройки, НастройкиОтчета, "КонтрольЖурналаРегистрации");
	...
	НастройкиВарианта = общ.ОписаниеВарианта(Настройки, НастройкиОтчета, "ПродолжительностьРаботыРегламентныхЗаданий");
...
Функция ДанныеИзЖурналаРегистрации(ПараметрыОтчета) Экспорт
	...
	ДатыВЧасовомПоясеСервера = общ.СвойствоСтруктуры(ПараметрыОтчета, "ДатыВЧасовомПоясеСервера", Ложь);
	Если ДатыВЧасовомПоясеСервера Тогда
		СмещениеВремениСервера = 0;
	Иначе
		СмещениеВремениСервера = общ.СмещениеВремениСервера();
	КонецЕсли;
...
Функция УникальныеИдентификаторыПользователей(ИдентификаторыПользователей)
	...
	общ.ДополнитьМассив(МассивУникальныеИдентификаторыПользователей,
	...
	Для Каждого Элемент Из МассивУникальныеИдентификаторыПользователей Цикл
		...
		Если ЗначениеЗаполнено(Элемент) Тогда
			...
			ИдентификаторПользователяИБ = общ.ЗначениеРеквизитаОбъекта(ИмяПользователяСсылка, "ИдентификаторПользователяИБ");
...
Функция ИмяПользователяИБ(ПользовательСсылка) Экспорт
	УстановитьПривилегированныйРежим(Истина);
	ИдентификаторПользователяИБ = общ.ЗначениеРеквизитаОбъекта(ПользовательСсылка, "ИдентификаторПользователяИБ");
		...
		Для Каждого СтрокаОдновременноСессий Из ОдновременноСессий Цикл
			...
			Для Каждого Элемент Из МассивРегламентныхЗаданий Цикл
				Если Не ТипЗнч(Элемент) = Тип("Число")
					...
					Область.Параметры.СписокРегламентныхЗаданий = 
						СтрШаблон(
							НСтр("ru = '%1 (сеанс %2)'"),
							Элемент,
...
Функция ДанныеДляОтчетаПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения)
	...
	СписокВсехРегламентныхЗаданий = НайтиЗадания(Новый Структура);
	...
	Для Каждого СтрокаДанныхЖурналаРегистрации Из ДанныеЖурналаРегистрации Цикл 
		...
		Если ТекущаяДата <> ДатаОдновременноРегламентныхЗаданий Тогда
			Если СтрокаТаблицы <> Неопределено Тогда
				...
				СтрокаТаблицы.ДатаОдновременноРегламентныхЗаданий = 
					СтрШаблон(
						НСтр("ru = '%1 - %2'"),
...
Функция ПредставлениеИнформационнойБазы()
	...
	Если общ.ИнформационнаяБаза_Файловая(СтрокаСоединенияСБД) Тогда
...
Функция ИнформацияПоОшибкамЖурналаРегистрации(Знач ДатаНачала, Знач ДатаОкончания, СмещениеВремениСервера)
	...
	структураОтбор.Вставить("Уровень", УровниРегистрацииОшибок);
	
	ДатаНачала = ДатаНачала + СмещениеВремениСервера;
	структураОтбор.Вставить("ДатаНачала", ДатаНачала);
	
	ДатаОкончания = ДатаОкончания + СмещениеВремениСервера;
	структураОтбор.Вставить("ДатаОкончания", ДатаОкончания);
	
	// обм. Отобор только по ошибкам подсистемы 'обм'
	именаСобытий = обм.ИмяСобытияЖурналаРегистрации();
	структураОтбор.Вставить("Событие", именаСобытий);
	
	таблицаПриемник = Новый ТаблицаЗначений;
	
	УстановитьПривилегированныйРежим(Истина);
	ВыгрузитьЖурналРегистрации(таблицаПриемник, структураОтбор);
	УстановитьПривилегированныйРежим(Ложь);
...
//
// Из модуля РегламентныеЗаданияСервер
//
Функция НайтиЗадания(Отбор) Экспорт

	КопияОтбора = общ.СкопироватьРекурсивно(Отбор); // см. НайтиЗадания.Отбор
