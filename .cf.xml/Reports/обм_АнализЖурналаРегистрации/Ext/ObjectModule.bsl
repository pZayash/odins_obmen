...
Процедура ПриЗагрузкеВариантаНаСервере(Форма, НовыеНастройкиКД) Экспорт
	Если общ.СмещениеВремениСервера() = 0 Тогда
...
Процедура ПриКомпоновкеРезультата(ДокументРезультат, ДанныеРасшифровкиОбъект, СтандартнаяОбработка, АдресХранилища)
	...
	Если ВариантОтчета = "КонтрольЖурналаРегистрации" Тогда
		РезультатФормированияОтчета = Отчеты.обм_АнализЖурналаРегистрации.
	...
	Иначе
		...
		ДанныеОтчета = Отчеты.обм_АнализЖурналаРегистрации.ДанныеИзЖурналаРегистрации(ПараметрыОтчета);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	ИначеЕсли ВариантОтчета = "АктивностьПользователя" Тогда
		...
		Если Отчеты.обм_АнализЖурналаРегистрации.ИмяПользователяИБ(Пользователь) = Неопределено Тогда
			общ.СообщитьПользователю(
	...
	ИначеЕсли ВариантОтчета = "АнализАктивностиПользователей" Тогда
		...
		Если ТипЗнч(ПользователиИГруппы) = Тип("СправочникСсылка.Пользователи") Тогда
			...
			Если Отчеты.обм_АнализЖурналаРегистрации.ИмяПользователяИБ(ПользователиИГруппы) = Неопределено Тогда
				общ.СообщитьПользователю(
					НСтр("ru = 'Формирование отчета возможно только для пользователя, которому указано имя для входа в программу.'"), , );
				Отказ = Истина;
				Возврат;
...
Процедура ПродолжительностьРаботыРегламентныхЗаданий(НастройкиОтчета, ДокументРезультат, КомпоновщикНастроек, СмещениеВремениСервера)
	...
	РезультатФормированияОтчета =
		Отчеты.обм_АнализЖурналаРегистрации.СформироватьОтчетПоПродолжительностиРаботыРегламентныхЗаданий(ПараметрыЗаполнения);
...
Функция СписокВсехРегламентныхЗаданий()
	...
	СписокРегламентныхЗаданий = Отчеты.обм_АнализЖурналаРегистрации.НайтиЗадания(Новый Структура); // РегламентныеЗаданияСервер.НайтиЗадания(Новый Структура)
