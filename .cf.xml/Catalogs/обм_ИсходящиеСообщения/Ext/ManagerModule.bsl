...
// Создание исходящего сообщения по ссылки источника
// с помощью переопределяемых обрабочтиков (см. Справончик.общ_Обработчики)
//
// Параметры:
	...
	Попытка
		...
		Для Каждого обработчик Из реализацииИнтерфейса Цикл
			...
			Для Каждого исходящееОбъект Из исходящие Цикл
				Если Ложь Тогда исходящееОбъект = Справочники.обм_ИсходящиеСообщения.СоздатьЭлемент() КонецЕсли;
				исходящееОбъект.Конструктор(пИсточникСсылка, , обработчик); // Дозаполнение реквизитов исходящего
