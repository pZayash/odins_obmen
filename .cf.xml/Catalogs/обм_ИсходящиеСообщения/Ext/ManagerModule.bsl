...
Функция Создать(пСсылки) Экспорт
	...
	ссылкиСозданных = Новый Массив;
	Если реализацииИнтерфейса.Количество() = 0 Тогда
		Возврат ссылкиСозданных;
		
	КонецЕсли;
	
	Для Каждого ссылкаИсточника Из пСсылки Цикл
		
		Если Не общ.ЭтоЗаполненнаяСсылка(ссылкаИсточника) Тогда
			Продолжить;
			
		КонецЕсли;
		
		исходящие = интерфейс.ИсходящееСообщение_Создать(ссылкаИсточника);
		...
		Для Каждого исходящееОбъект Из исходящие Цикл
			...
			ссылкиСозданных.Добавить(исходящееОбъект.Ссылка);
...
Функция ИсходящееСообщение_Отправить(пИсходящееСообщение, пУзел, пОбработчик = Неопределено) Экспорт
	...
	отправитель.ИсходящееСообщение_Отправить(пИсходящееСообщение, пУзел, пОбработчик);
	
	записьОбОтправке = РегистрыСведений.обм_ОтправленныеСообщения
