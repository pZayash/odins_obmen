...
Процедура Очистка_ВходящиеСообщения() Экспорт
	...
	срокДней = Настройки_СрокХраненияДней(ИмяНастройкиОчисткаВходящих());
...
Процедура Очистка_ИсходящиеСообщения() Экспорт
	...
	срокДней = Настройки_СрокХраненияДней(ИмяНастройкиОчисткаИсходящих());
	датаГраницы = ТекущаяДатаСеанса() - срокДней * 86400;
...
// Срок хранения в днях из предопределённого элемента обм_Настройки.
//
Функция Настройки_СрокХраненияДней(пИмяПредопределенного) Экспорт

	общ.Проверка_Тип(пИмяПредопределенного, Тип("Строка"));

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off
	ссылка = Справочники.обм_Настройки[пИмяПредопределенного];
	срок = ссылка.СрокХраненияДней;
	УстановитьПривилегированныйРежим(Ложь);
...
// Записать срок хранения в предопределённый элемент настроек.
//
Процедура Настройки_УстановитьСрокХраненияДней(пИмяПредопределенного, пСрокДней) Экспорт

	общ.Проверка_Тип(пИмяПредопределенного, Тип("Строка"));
	общ.Проверка_Тип(пСрокДней, Тип("Число"));

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off
	объект = Справочники.обм_Настройки[пИмяПредопределенного].ПолучитьОбъект();
...
Функция ИмяНастройкиОчисткаВходящих() Экспорт

	Возврат "ОчисткаВходящих";

КонецФункции

Функция ИмяНастройкиОчисткаИсходящих() Экспорт

	Возврат "ОчисткаИсходящих";

КонецФункции

#КонецОбласти // ОчисткаОчередей
...
#Область ОчисткаОчередейСлужебные

Функция РазмерПачкиОчистки()
...
Процедура Очереди_ОчиститьВходящие(пДатаГраницы)
	...
	размерПачки = РазмерПачкиОчистки();
	удаленоСообщений = 0;

	Пока Истина Цикл
		сообщения = Очереди_ВыбратьВходящиеСообщения(пДатаГраницы, размерПачки);
		Если сообщения.Количество() = 0 Тогда
			Прервать;

		КонецЕсли;

		пакеты = Очереди_ВыбратьПакетыПоВладельцам(сообщения);
		Очереди_УдалитьСтатусыВходящих(сообщения);
		Очереди_УдалитьПриемникиПоПакетам(пакеты);
		Очереди_УдалитьЗагруженныеПакеты(пакеты);
		Очереди_УдалитьСсылкиСправочника(пакеты);
		Очереди_УдалитьСсылкиСправочника(сообщения);

		удаленоСообщений = удаленоСообщений + сообщения.Количество();
...
Процедура Очереди_ОчиститьИсходящие(пДатаГраницы)
	...
	Пока Истина Цикл
		сообщения = Очереди_ВыбратьИсходящиеСообщения(пДатаГраницы, размерПачки);
		Если сообщения.Количество() = 0 Тогда
			Прервать;

		КонецЕсли;

		Очереди_УдалитьРегистрПоИзмерениюСообщения(
			"РегистрСведений.обм_КОтправке", сообщения);
		Очереди_УдалитьРегистрПоИзмерениюСообщения(
			"РегистрСведений.обм_ОтправленныеСообщения", сообщения);
		Очереди_УдалитьСсылкиСправочника(сообщения);
...
Функция Очереди_ВыбратьВходящиеСообщения(пДатаГраницы, пРазмерПачки)
	...
	|ИЗ
	|	Справочник.обм_ВходящиеСообщения КАК Очередь
	|ГДЕ
	|	Очередь.ДатаВремя < &ДатаГраницы
	|
	|УПОРЯДОЧИТЬ ПО
	|	Очередь.ДатаВремя";
	запрос.Текст = СтрЗаменить(запрос.Текст, "100", Формат(пРазмерПачки, "ЧГ=0"));
...
Функция Очереди_ВыбратьИсходящиеСообщения(пДатаГраницы, пРазмерПачки)
	...
	|	Очередь.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.обм_ИсходящиеСообщения КАК Очередь
	|ГДЕ
	|	Очередь.ДатаВремя < &ДатаГраницы
	|
	|УПОРЯДОЧИТЬ ПО
...
Функция Очереди_ВыбратьПакетыПоВладельцам(пСообщения)

	Если пСообщения.Количество() = 0 Тогда
	...
	|ГДЕ
	|	Пакеты.Владелец В (&Сообщения)";
	запрос.УстановитьПараметр("Сообщения", пСообщения);

	Возврат запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Ссылка");

КонецФункции

Процедура Очереди_УдалитьСтатусыВходящих(пСообщения)

	Для Каждого сообщение Из пСообщения Цикл
		набор = РегистрыСведений.обм_ВходящиеСообщения_Статусы.СоздатьНаборЗаписей();
		набор.Отбор.ВходящееСообщение.Установить(сообщение);
		набор.Записать();
...
Процедура Очереди_УдалитьПриемникиПоПакетам(пПакеты)

	Если пПакеты.Количество() = 0 Тогда
		Возврат;
	...
	запрос.Текст =
	"ВЫБРАТЬ
	|	Приемники.Узел КАК Узел,
	|	Приемники.Категория КАК Категория,
	|	Приемники.Идентификатор КАК Идентификатор,
	|	Приемники.Приемник КАК Приемник
	|ИЗ
	|	РегистрСведений.обм_Приемники КАК Приемники
	|ГДЕ
	|	Приемники.ПакетДанных В (&Пакеты)";
	запрос.УстановитьПараметр("Пакеты", пПакеты);

	выборка = запрос.Выполнить().Выбрать();
	Пока выборка.Следующий() Цикл
		менеджер = РегистрыСведений.обм_Приемники.СоздатьМенеджерЗаписи();
		менеджер.Узел = выборка.Узел;
		менеджер.Категория = выборка.Категория;
		менеджер.Идентификатор = выборка.Идентификатор;
		менеджер.Приемник = выборка.Приемник;
		менеджер.Удалить();
...
Процедура Очереди_УдалитьЗагруженныеПакеты(пПакеты)
	...
	запрос.Текст =
	"ВЫБРАТЬ
	|	Загруженные.Приемник КАК Приемник
	|ИЗ
	|	РегистрСведений.обм_Приемники_ЗагруженныеПакетыДанных КАК Загруженные
	|ГДЕ
	|	Загруженные.ПакетДанных В (&Пакеты)";
	запрос.УстановитьПараметр("Пакеты", пПакеты);

	выборка = запрос.Выполнить().Выбрать();
	Пока выборка.Следующий() Цикл
		набор = РегистрыСведений.обм_Приемники_ЗагруженныеПакетыДанных.СоздатьНаборЗаписей();
		набор.Отбор.Приемник.Установить(выборка.Приемник);
		набор.Записать();
...
Процедура Очереди_УдалитьРегистрПоИзмерениюСообщения(пИмяРегистра, пСообщения)

	Для Каждого сообщение Из пСообщения Цикл
		Если пИмяРегистра = "РегистрСведений.обм_КОтправке" Тогда
			набор = РегистрыСведений.обм_КОтправке.СоздатьНаборЗаписей();
			набор.Отбор.ИсходящееСообщение.Установить(сообщение);
			набор.Записать();

		ИначеЕсли пИмяРегистра = "РегистрСведений.обм_ОтправленныеСообщения" Тогда
			набор = РегистрыСведений.обм_ОтправленныеСообщения.СоздатьНаборЗаписей();
			набор.Отбор.ИсходящееСообщение.Установить(сообщение);
			набор.Записать();
...
Процедура Очереди_УдалитьСсылкиСправочника(пСсылки)

	Если пСсылки.Количество() = 0 Тогда
		Возврат;

	КонецЕсли;

	УдалитьОбъекты(пСсылки, Ложь);

КонецПроцедуры

#КонецОбласти // ОчисткаОчередейСлужебные
