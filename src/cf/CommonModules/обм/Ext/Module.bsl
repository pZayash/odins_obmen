...
//
// === MODULE_CONTRACT ===
// Именование методов (псевдообъекты):
//   ИмяПсевдоОбъекта_Реквизит_Действие
// Пример: ВходящееСообщение_Параметр_Значение, ОчисткаВходящих_СрокХраненияДней_Установить
// Явных объектов метаданных под псевдоимена может не быть — стиль на случай выделения.
// Канон: docs/ai/code-standards-kpsr.md § «Именование по псевдообъектам»
// === END ===
...
Функция Альфа_JSON_ПоОбъекту(пИсточник) Экспорт // BSLLS:LatinAndCyrillicSymbolInWord-off
	...
	образ = Неопределено;
	Если общ.ЭтоЗаполненнаяСсылка(пИсточник) Тогда
		образ = Альфа_Образ_ПоСсылке(пИсточник);
		
	ИначеЕсли общ.ЭтоКоллекцияКлючЗначение(пИсточник) Тогда
		образ = пИсточник;
		
	Иначе
		ВызватьИсключение "Создание образа в формате 'обм.Альфа' не поддерживатеся. Ожидается ссылка, соответствие или структура";
...
Функция Альфа_РазвернутьСсылки(пКоллекция, Знач пУровень, Знач пИсключаяСвойства = Неопределено) Экспорт
	...
	развернутаяКоллекция = Новый Соответствие;
	Для Каждого пара Из пКоллекция Цикл
		...
		этоОбразТЧ = 	общ.ЭтоЗаполненныйМассив(значение) 
					И	ТипЗнч(метаданныеТЧ) = Тип("КоллекцияОбъектовМетаданных")
					И метаданныеТЧ.Найти(пара.Ключ) <> Неопределено
					И Истина;
					
		Если этоОбразТЧ Тогда
			развернутыйОбразТЧ = Новый Массив;
			Для Каждого образРяда Из значение Цикл
				Если общ.ЭтоКоллекцияКлючЗначение(образРяда) Тогда
					// Для табличных частей уровень не уменьшаем, считаем что строки табличной части находяться на том же уровне
					// что и обычные реквизиты образа
					развернутыйОбразРяда = Альфа_РазвернутьСсылки(образРяда, пУровень, стрИсключитьСвойства);
					развернутыйОбразТЧ.Добавить(развернутыйОбразРяда);

				Иначе
					развернутыйОбразТЧ.Добавить(образРяда);

				КонецЕсли;

			КонецЦикла;
			образРяда = Неопределено;
			развернутаяКоллекция.Вставить(пара.Ключ, развернутыйОбразТЧ);

			Продолжить;
			
		КонецЕсли; // ЭтоОбразТЧ
			
		Если общ.ЭтоСсылка(значение) Тогда
			значение = Альфа_Образ_ПоСсылке(значение, стрИсключитьСвойства);
			
		КонецЕсли;
		
		// Обработка вложенных коллекций, уровень уменьшается
		Если общ.ЭтоКоллекцияКлючЗначение(значение) Тогда
			следующийУровень = пУровень - 1;
			обработаннаяКоллекция = Альфа_РазвернутьСсылки(значение, следующийУровень, стрИсключитьСвойства);

			развернутаяКоллекция.Вставить(пара.Ключ, обработаннаяКоллекция);
	...
	Для Каждого пара Из развернутаяКоллекция Цикл
...
#Область Очереди_Очистка

Процедура Очистка_ВходящиеСообщения() Экспорт

	срокДней = ОчисткаВходящих_СрокХраненияДней_Значение();
	датаГраницы = ТекущаяДатаСеанса() - срокДней * 86400;
	Очереди_Очистить(ВидОчередиВходящие(), датаГраницы);

КонецПроцедуры

Процедура Очистка_ИсходящиеСообщения() Экспорт

	срокДней = ОчисткаИсходящих_СрокХраненияДней_Значение();
	датаГраницы = ТекущаяДатаСеанса() - срокДней * 86400;
	Очереди_Очистить(ВидОчередиИсходящие(), датаГраницы);

КонецПроцедуры

// Удаляет сообщения очереди и связанные служебные данные без контроля ссылок.
//
// Параметры:
//  пВидОчереди - Строка - "Входящие" или "Исходящие"
//  пДатаГраницы - Дата - удалять записи с ДатаВремя строго меньше границы
//
Процедура Очереди_Очистить(пВидОчереди, пДатаГраницы) Экспорт

	общ.Проверка_Тип(пВидОчереди, Тип("Строка"));
	общ.Проверка_Тип(пДатаГраницы, Тип("Дата"));

	Если пВидОчереди = ВидОчередиВходящие() Тогда
		Очереди_ОчиститьВходящие(пДатаГраницы);

	ИначеЕсли пВидОчереди = ВидОчередиИсходящие() Тогда
		Очереди_ОчиститьИсходящие(пДатаГраницы);

	Иначе
		ВызватьИсключение СтрШаблон("Неизвестный вид очереди: %1", пВидОчереди);

	КонецЕсли;

КонецПроцедуры

// Полная очистка регистра обм_Измененные (без фильтра по дате).
//
Процедура Измененные_ОчиститьВсе() Экспорт

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off

	набор = РегистрыСведений.обм_Измененные.СоздатьНаборЗаписей();
	набор.Записать();
...
// Срок хранения очереди входящих (предопределённый обм_Настройки.ОчисткаВходящих).
//
Функция ОчисткаВходящих_СрокХраненияДней_Значение() Экспорт

	Возврат Настройки_СрокХраненияДнейПрочитать(Справочники.обм_Настройки.ОчисткаВходящих);

КонецФункции

// Записать срок хранения очереди входящих.
//
Процедура ОчисткаВходящих_СрокХраненияДней_Установить(пСрокДней) Экспорт

	Настройки_СрокХраненияДнейЗаписать(Справочники.обм_Настройки.ОчисткаВходящих, пСрокДней);

КонецПроцедуры

// Срок хранения очереди исходящих (предопределённый обм_Настройки.ОчисткаИсходящих).
//
Функция ОчисткаИсходящих_СрокХраненияДней_Значение() Экспорт

	Возврат Настройки_СрокХраненияДнейПрочитать(Справочники.обм_Настройки.ОчисткаИсходящих);

КонецФункции

// Записать срок хранения очереди исходящих.
//
Процедура ОчисткаИсходящих_СрокХраненияДней_Установить(пСрокДней) Экспорт

	Настройки_СрокХраненияДнейЗаписать(Справочники.обм_Настройки.ОчисткаИсходящих, пСрокДней);

КонецПроцедуры

Функция ВидОчередиВходящие() Экспорт

	Возврат "Входящие";

КонецФункции

Функция ВидОчередиИсходящие() Экспорт

	Возврат "Исходящие";

КонецФункции

Функция СрокХраненияДнейПоУмолчанию()

	Возврат 90;

КонецФункции

Функция Настройки_СрокХраненияДнейПрочитать(пСсылкаНастройки)

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off
	срок = пСсылкаНастройки.СрокХраненияДней;
	УстановитьПривилегированныйРежим(Ложь);

	Если Не ЗначениеЗаполнено(срок) Или срок < 1 Тогда
		Возврат СрокХраненияДнейПоУмолчанию();
...
Процедура Настройки_СрокХраненияДнейЗаписать(пСсылкаНастройки, пСрокДней)

	общ.Проверка_Тип(пСрокДней, Тип("Число"));

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off
	объект = пСсылкаНастройки.ПолучитьОбъект();
	объект.СрокХраненияДней = пСрокДней;
...
Процедура Очистка_СообщитьПрогресс(пУдалено, пКонтекст)

	Если Не общ_Кэш.ЕстьБСП() Тогда
		Возврат;
	...
	// Обманка проверки модуля
	Если Ложь Тогда
		ДлительныеОперации = Неопределено;

	КонецЕсли;

	ДлительныеОперации.СообщитьПрогресс( ,
		СтрШаблон("Очистка %1: удалено %2", пКонтекст, Формат(пУдалено, "ЧГ=0")));

КонецПроцедуры

Процедура Очереди_ОчиститьВходящие(пДатаГраницы)

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off

	// Статусы: ведущее измерение - платформа снимет при удалении сообщения.
	// обм_Приемники - сопоставление, не чистим.
	// ЗагруженныеПакетыДанных: ПакетДанных не ведущее - чистим в транзакции сообщения.

	удаленоСообщений = 0;
	запрос = Новый Запрос;
	запрос.Текст =
	"ВЫБРАТЬ
	|	ОчередьВходящие.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.обм_ВходящиеСообщения КАК ОчередьВходящие
	|ГДЕ
	|	ОчередьВходящие.ДатаВремя < &ДатаГраницы
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОчередьВходящие.ДатаВремя";
	запрос.УстановитьПараметр("ДатаГраницы", пДатаГраницы);
	выборкаСообщений = запрос.Выполнить().Выбрать();
	Пока выборкаСообщений.Следующий() Цикл
		Если Очереди_УдалитьВходящееСообщение(выборкаСообщений.Ссылка) Тогда
			удаленоСообщений = удаленоСообщений + 1;
			Очистка_СообщитьПрогресс(удаленоСообщений, "входящие");
...
Процедура Очереди_ОчиститьИсходящие(пДатаГраницы)

	УстановитьПривилегированныйРежим(Истина); // BSLLS:SetPrivilegedMode-off

	// КОтправке / Отправленные: ведущее измерение - платформа снимет при удалении.

	удаленоСообщений = 0;
	запрос = Новый Запрос;
	запрос.Текст =
	"ВЫБРАТЬ
	|	ОчередьИсходящие.Ссылка КАК Ссылка
	|ИЗ
	|	Справочник.обм_ИсходящиеСообщения КАК ОчередьИсходящие
	|ГДЕ
	|	ОчередьИсходящие.ДатаВремя < &ДатаГраницы
	|
	|УПОРЯДОЧИТЬ ПО
	|	ОчередьИсходящие.ДатаВремя";
	запрос.УстановитьПараметр("ДатаГраницы", пДатаГраницы);
	выборкаСообщений = запрос.Выполнить().Выбрать();
	Пока выборкаСообщений.Следующий() Цикл
		Если Очереди_УдалитьИсходящееСообщение(выборкаСообщений.Ссылка) Тогда
			удаленоСообщений = удаленоСообщений + 1;
			Очистка_СообщитьПрогресс(удаленоСообщений, "исходящие");
...
// Удаляет входящее сообщение и его пакеты одной транзакцией.
// Возвращает Истина, если зафиксировано; Ложь при откате.
//
Функция Очереди_УдалитьВходящееСообщение(пСсылка)

	НачатьТранзакцию();
	Попытка
		запросЗагруженных = Новый Запрос;
		запросЗагруженных.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ЗагруженныеРС.Период КАК Период,
		|	ЗагруженныеРС.Приемник КАК Приемник
		|ИЗ
		|	РегистрСведений.обм_Приемники_ЗагруженныеПакетыДанных КАК ЗагруженныеРС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.обм_ПакетыДанных КАК ПакетыДанных
		|		ПО ЗагруженныеРС.ПакетДанных = ПакетыДанных.Ссылка
		|ГДЕ
		|	ПакетыДанных.Владелец = &Сообщение";
		запросЗагруженных.УстановитьПараметр("Сообщение", пСсылка);
		выборкаЗагруженных = запросЗагруженных.Выполнить().Выбрать();
		Пока выборкаЗагруженных.Следующий() Цикл
			// Ключ: Период + Приемник. Отбор только по Приемник сотрёт всю историю.
			запись = РегистрыСведений.обм_Приемники_ЗагруженныеПакетыДанных.СоздатьМенеджерЗаписи();
			запись.Период = выборкаЗагруженных.Период;
			запись.Приемник = выборкаЗагруженных.Приемник;
			запись.Удалить();
		...
		|ГДЕ
		|	ПакетыДанных.Владелец = &Сообщение";
		запросПакетов.УстановитьПараметр("Сообщение", пСсылка);
		выборкаПакетов = запросПакетов.Выполнить().Выбрать();
		Пока выборкаПакетов.Следующий() Цикл
			выборкаПакетов.Ссылка.ПолучитьОбъект().Удалить();

		КонецЦикла;

		пСсылка.ПолучитьОбъект().Удалить();
		ЗафиксироватьТранзакцию();

	Исключение
		ОтменитьТранзакцию();
		Очереди_Очистка_ЗаписатьОшибку(ИнформацияОбОшибке(), пСсылка);
		Возврат Ложь;
...
// Удаляет исходящее сообщение одной транзакцией.
// Связанные регистры с ведущим измерением снимает платформа.
//
Функция Очереди_УдалитьИсходящееСообщение(пСсылка)
...
Процедура Очереди_Очистка_ЗаписатьОшибку(пИнфоОшибки, пДанные)

	общ.Ошибка_ВЖурналРегистрации(
		ИмяСобытияЖурналаРегистрации("ОчисткаОчереди"),
		пИнфоОшибки,
		пДанные);

КонецПроцедуры

#КонецОбласти // Очереди_Очистка
...
Функция ЭтоОбъектПодсистемыОбм(пОбъект)
	...
	результат =	ТипЗнч(пОбъект) = Тип("СправочникОбъект.обм_ВходящиеСообщения") Или ТипЗнч(пОбъект) = Тип(
		...
		Или ТипЗнч(пОбъект) = Тип("СправочникОбъект.обм_Настройки");
	Возврат результат;
