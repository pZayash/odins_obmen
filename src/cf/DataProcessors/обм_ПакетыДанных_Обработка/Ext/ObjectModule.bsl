...
Функция ПакетДанных_Обработать(пакетДанныхСсылка) Экспорт
	...
	Попытка
		...
		Если общ.ОбъектСсылкаДругое(приемникОбъект) = "Объект" И приемникОбъект.Модифицированность() Тогда
			...
			мПриемниковЗаполнено = мПриемниковЗаполнено + 1;
...
Процедура Ошибка_ВЖурнал(
	...
	Если общ.ЭтоЗаполненнаяСсылка(пПакетДанных, Тип("СправочникСсылка.обм_ПакетыДанных")) Тогда
		ссылкиДанных.Добавить(пПакетДанных.Владелец); // СправочникСсылка.обм_ВходящиеСообщения
