...
Процедура ПоказатьРазмерОчереди(Команда)
	...
	// поэтому используется условно устаревший метод ТекущаяДата
	представлениеДатыКлиента = Формат(ТекущаяДата(), "ДФ=HH:mm:ss"); // BSLLS:DeprecatedCurrentDate-off
	
	текстЗаголовка = СтрШаблон("%1 Размер очереди %2. Нажмите здесь чтобы пересчитать",
			представлениеДатыКлиента, // %1
			размерОчереди		);
