...
скрипт делает `/UpdateDBCfg` (платформа не создаёт дистрибутив, пока
основная конфигурация не совпадает с конфигурацией БД). Если XML ещё
не в ИБ — сначала `./load-changed-files.sh -U`.
